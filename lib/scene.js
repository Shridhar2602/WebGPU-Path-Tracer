...
export class Scene {
	...
	create_meshes() {
		...
		this.meshes.push(
			new Mesh(this.mesh_data["bunny"], this.triangle_offset, this.global_id++, this.mesh_id++, this.triangle_id, this.add_material("glassBox", 0, [0.95, 0.95, 0.95], [0.95, 0.95, 0.95], [0, 0, 0], 0, 0, 1.5))
		)
		...
		// 	// this.meshes[0].transform.rotate(Math.PI / 2, [0, 1, 0]),
		// 	// this.meshes[0].transform.rotate(-Math.PI / 8, [1, 0, 0]),
		// 	this.meshes[0].transform.translate(-3, 0, 0),
		// )
	...
	linear_bvh(bvh) {
		...
		bvh.id = this.bvh_id++;
		if(bvh.obj != null) {
			this.bvh_array.push([bvh.bbox.min[0], bvh.bbox.min[1], bvh.bbox.min[2], -1, bvh.bbox.max[0], bvh.bbox.max[1], bvh.bbox.max[2], bvh.obj.type, bvh.startID, bvh.triCount, bvh.miss_node, bvh.hit_node]);
		}
		else {
			this.bvh_array.push([bvh.bbox.min[0], bvh.bbox.min[1], bvh.bbox.min[2], -1, bvh.bbox.max[0], bvh.bbox.max[1], bvh.bbox.max[2], -1, -1, -1, bvh.miss_node, bvh.hit_node]);
		}
