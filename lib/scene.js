...
export class Scene {
	...
	create_spheres() {
		...
		// dummy sphere
		this.add_material("default", 0, [1, 1, 0.4], [0, 0, 0], 0, 0);
		// objs.push(this.sphere([0, 0, 2], 0.3, this.material_dict["default"]));
		...
		sphere2.transform.update(
			// sphere2.transform.translate(0, 0.5, -0.5),
	...
	create_quads() {
		...
		let q4 = new Quad([-0.35, 1, -0.3],[0.7, 0, 0], [0, 0, 0.6], this.obj_id++, this.add_material("light", 0, [0, 0, 0], [1, 1, 1], 0, 0)); //light  // 13
		...
		let q7 = new Quad([1, -1, 1],      [-2, 0, 0],  [0, 2, 0],   this.obj_id++, this.add_material("fWall", 0, [0, 0, 1], [1, 1, 1], 0, 0)); //front
	...
	async init_mesh_data() {
		...
		this.mesh_data["cubeNormal"] = await ObjReader.load_model("./lib/cubeNormal.obj");
		this.mesh_data["monkey1"] = await ObjReader.load_model("./lib/monkey_968.obj");
	}

	create_meshes() {

		let m1 = new Mesh(this.mesh_data["monkey1"], this.triangle_offset, this.obj_id++, this.material_dict["default"]);
		this.triangle_offset += m1.numTriangle;
		
		this.triangles = [m1.triangles];
		this.meshes = [m1.mesh];
